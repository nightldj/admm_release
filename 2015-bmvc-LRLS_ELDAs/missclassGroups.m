...
%--------------------------------------------------------------------------
% this code is accelerated by Xiao Shijie (xiaoshijiehithonor@gmail.com) usig Hungarian algorithm. Oct.2013.
%--------------------------------------------------------------------------


function [miss,index] = missclassGroups_by_Jerry(Segmentation,RefSegmentation,ngroups, version)
% version |   str   |   if          = 'ori" ,    the ori verion' 
%                             elseif   = 'fast',    the accelerated version  using Hungarian algo.  (the default one)

% Output:
%               miss : #missclassified samples
%               index: the best mach
...
% if index(i) = k, it means the i-th cluster is assigned using the k-th class
...
switch version
    ...
    case 'fast'% It requires that length(unique(RefSegmentation)) = length(unique( RefSegmentation ))
        nclass          = ngroups;
        ref_mat        = Jerry_ind2indmat( RefSegmentation , nclass );%nsample x nclass
        cluster_mat = Jerry_ind2indmat( Segmentation    , nclass );
        A                  = double(cluster_mat') * double( ref_mat );% nclass x nclass
%         tic;
        [C,T]=hungarian(-A); %s.t. T = trace(A(C,:)) is minimized over all possible assignments.
%         toc
        n_correct = -T;
        miss  = length(RefSegmentation) - n_correct;
        index = C;
    otherwise
...
function indmat = Jerry_ind2indmat( ind , nclass)
if ~exist('nclass','var')
    unique_ind = unique( ind );
    nclass     = length( unique_ind );
end
if ~exist('unique_ind','var')
    unique_ind = 1:nclass;%unique( ind );
end
...
indmat = false( nsample, nclass);
for which = 1 : nclass
    indmat( :, which ) = (ind == unique_ind( which ) );
end
